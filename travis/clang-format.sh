...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
    ...
    git remote add upstream https://github.com/GodotVR/godot_openvr \
...
FILES=$(git diff-tree --no-commit-id --name-only -r $RANGE | grep -v godot_headers/ | grep -v ovr_sdk_win/ | grep -v glad/ | grep -E "\.(c|h|cpp|hpp|cc|hh|cxx|m|mm|inc|java|glsl)$")
echo "Checking files:\n$FILES"
